    ...
    for j in range(len(U)):
        for k in range(len(A)):
            ...
            for i in range(len(vel)):
                ...
                ########### 2 - Radiadores em paralelo ###########
                Cf1 = m_ar(vel[i], A[k])*c_ar
                Cf2 = m_ar(vel[i], A[k])*c_ar
                Cq  = 0.5*m_H20(rpm[i])*c_H20
                ...
                # Taxa de calor cedido pelo Radiador 2
                e2     = 1 - np.exp(((NTU2**0.22)/C2)*(np.exp(-C2*(NTU2**0.78)) - 1))
                Q2     = e2*Q_max2

                # Diminuição da temperatura da água na saída dos radiadores
                T1 = T - 2*Q1/(m_H20(rpm[i])*c_H20)
                T2 = T - 2*Q2/(m_H20(rpm[i])*c_H20)

                # Encontrando a entalpia na saída de cada radiador
                h1 = entalpia(T1)
                h2 = entalpia(T2)

                # Encontrando a entalpia da mistura 
                h3 = 0.5*(h1 + h2)

                # Encontrando a temperatura da mistura
                T = temperatura(h3)
...
plt.title('Temperatura máxima com dois radiadores em paralelo')
